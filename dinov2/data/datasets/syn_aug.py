...
if __name__ == "__main__":
    root = '/SSD2T/synthetic0302/imagenet/view1'
    ...
    dataset = MultiCropDataset(root, transforms=transform, num_views=2)
    
    print(len(dataset))
    # from tqdm import tqdm
    # for i in tqdm(range(271431)):
    output, _ , paths = dataset.__getitem__(200101)
    print(_)
    print(paths)
    ...
    def visualize_images(tensor_images):
        ...
        for i, tensor_img in enumerate(global_crops):
            img = T.ToPILImage()(tensor_img)
            plt.figure()
            plt.imshow(img)
            plt.title(f"Global Crop {i+1}")
            plt.axis('off')
            plt.savefig(f"global_crop_{i+1}.png")
            plt.close()

    # Visualize all images in global crops
    visualize_images(global_crops)
