...
def get_prompt(dataset, category,):

    if dataset == 'ucf101_frames':
        prompt = origion_ucf_attribute.format(category)
    elif dataset == 'eurosat':
        prompt = origion_eurosat_attribute.format(category, category)
    else:
        prompt = origion_attribute.format(category, category)
    ...
    for cls in classes:
        ...
        if cls not in attribute_dict.keys():
            while True:
                time.sleep(0.01)
                cls_def = cls
                messages = get_prompt(dataset, cls_def)
